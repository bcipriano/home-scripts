...
def _convert_with_handbrake(in_file, out_dir, file_num, total_files):
  ...
  with tempfile.NamedTemporaryFile() as fp:
    ...
    cmd = ('/Applications/HandBrakeCLI --preset "{preset}" --subtitle-lang-list eng '
           '--first-subtitle --input "{input}" --output "{output}" &> {filename} & echo $!').format(
               preset=OUTPUT_PRESET, input=in_file, output=out_file, filename=fp.name)
